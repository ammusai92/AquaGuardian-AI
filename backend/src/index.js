...
// ---- Serve compiled frontend if built (production mode) ----
...
app.use('/api', require('./monitor').router);
...
ensureAdmin()
  .then(() => {
    app.listen(PORT, () => console.log(`AquaGuardian API on http://localhost:${PORT}`));
    require('./monitor').startMonitor();
  })
